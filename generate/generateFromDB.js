...
const _BridgingTableHander= async (Bridgings)=>{ 
    for (let j = 0; j < Bridgings.length; j++) {
        await command.createRelation(Bridgings[j][0].REFERENCED_TABLE_NAME,Bridgings[j][1].REFERENCED_TABLE_NAME,'mtm');
    }
}
...
const _RelationHandler = async (foreignConstraint) =>{
    for(let j=0; j<foreignConstraint.length;j++){
        ...
        await command.createRelation(foreignConstraint[j].TABLE_NAME,foreignConstraint[j].REFERENCED_TABLE_NAME,relation);
    }
}
