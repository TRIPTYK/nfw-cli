        ...
        const question = [
          {
            type : 'input',
            name: 'models',
            message : `What are the two entities related(separate them by a coma)`,
            validate : (models) =>{
                if(!utils.modelFileExists(models[0])) return `${models[0]} doesn't exist`
                if(!utils.modelFileExists(models[1])) return `${models[1]} doesn't exist`
                else return true 
            },
            filter: data => data.split(',').map((value) => value.trim())
        }
        ];
        return inquirer.prompt(question);
    }
